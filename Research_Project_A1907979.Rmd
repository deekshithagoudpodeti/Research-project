...
model_coef_clean <- model_coef %>%
  ...
  mutate(
    # nicer labels (edit/extend as needed)
...
# --- Plot (object so we can save it twice)
p_coef <- ggplot(model_coef_clean, aes(x = label, y = estimate, fill = direction)) +
  geom_col(width = 0.75) +
  coord_flip() +
  scale_fill_manual(values = c("Positive" = "#D55E00", "Negative" = "#009E73")) + # CB-friendly
  labs(
...
# --- Save to PDF (best for LaTeX) and PNG (for slides)
